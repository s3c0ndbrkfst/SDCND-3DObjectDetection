...
def show_pcl(pcl):
    ...
    vis_pcl.create_window(window_name='Lidar Point Cloud')
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    lidar_pcl_copy[:, 0] = np.int_(
        np.floor(lidar_pcl_copy[:, 0] / bev_discrete))
    # step 3 : perform the same operation as in step 2 for the y-coordinates but make sure that no negative bev-coordinates occur
    lidar_pcl_copy[:, 1] = np.int_(
        np.floor(lidar_pcl_copy[:, 1] / bev_discrete) + (configs.bev_width + 1) / 2)
    ...
    idx_height = np.lexsort(
        (-lidar_pcl_copy[:, 2], lidar_pcl_copy[:, 1], lidar_pcl_copy[:, 0]))
    lidar_pcl_sort = lidar_pcl_copy[idx_height]
    # step 3 : extract all points with identical x and y such that only the top-most z-coordinate is kept (use numpy.unique)
    # also, store the number of points per x,y-cell in a variable named "counts" for use in the next task
    _, idx_height_unique, counts = np.unique(
        lidar_pcl_sort[:, 0:2], axis=0, return_index=True, return_counts=True)
    lidar_pcl_top = lidar_pcl_sort[idx_height_unique]
    # step 4 : assign the intensity value of each unique entry in lidar_top_pcl to the intensity map
    # make sure that the intensity is scaled in such a way that objects of interest (e.g. vehicles) are clearly visible
    # also, make sure that the influence of outliers is mitigated by normalizing intensity on the difference between the max. and min. value within the point cloud
    intensity_map[np.int_(lidar_pcl_top[:, 0]), np.int_(
        lidar_pcl_top[:, 1])] = lidar_pcl_top[:, 3] / (np.amax(lidar_pcl_top[:, 3])-np.amin(lidar_pcl_top[:, 3]))
    ...
    density_map[np.int_(lidar_pcl_top[:, 0]), np.int_(
        lidar_pcl_top[:, 1])] = normalizedCounts

    # assemble 3-channel bev-map from individual maps
    bev_map = np.zeros((3, configs.bev_height, configs.bev_width))
    bev_map[2, :, :] = density_map[:configs.bev_height,
                                   :configs.bev_width]  # r_map
    bev_map[1, :, :] = height_map[:configs.bev_height,
                                  :configs.bev_width]  # g_map
    bev_map[0, :, :] = intensity_map[:configs.bev_height,
                                     :configs.bev_width]  # b_map
